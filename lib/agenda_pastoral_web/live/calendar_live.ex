  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      <div class="space-y-6 max-w-6xl mx-auto animate-fade-in">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4 py-4 border-b border-base-200/50">
          <div>
            <h1 class="text-3xl font-black tracking-tight text-base-content text-center sm:text-left">
              Agenda Distrital
            </h1>
            <p class="text-xs sm:text-sm font-semibold opacity-60 text-center sm:text-left">
              Calendário de visitas do Pr. Raimundo Rosendo e atividades oficiais
            </p>
          </div>

          <div class="flex items-center gap-1.5 bg-base-200/50 p-1.5 rounded-xl border border-base-300">
            <button phx-click="prev_month" class="btn btn-sm btn-ghost btn-circle">
              <.icon name="hero-chevron-left" class="size-4" />
            </button>
            <span class="text-xs sm:text-sm font-black min-w-32 text-center text-base-content">
            ...
            <button phx-click="go_to_today" class="btn btn-xs sm:btn-sm btn-primary">
              Hoje
            </button>
          </div>
        </div>
        ...
        <%!-- Grid do Calendário --%>
        <div class="bg-base-100 border border-base-200/60 rounded-3xl overflow-hidden shadow-sm">
          <%!-- Cabeçalhos dos dias da semana --%>
          <div class="grid grid-cols-7 border-b border-base-200/50 bg-base-200/20 text-center py-2 text-[10px] sm:text-xs font-black tracking-wider opacity-70">
            <div>DOM</div>
            <div>SEG</div>
            <div>TER</div>
            <div>QUA</div>
          ...
          <%!-- Dias --%>
          <div class="grid grid-cols-7 divide-x divide-y divide-base-200/50 bg-base-100 border-t border-base-200/50">
            <div
              :for={{date, type} <- @grid_days}
              phx-click="select_date"
              phx-value-date={Date.to_iso8601(date)}
              class={[
                "min-h-16 sm:min-h-24 p-2 cursor-pointer transition-all relative flex flex-col justify-between hover:bg-primary/5",
                type != :curr && "bg-base-200/30 opacity-35",
                date == @selected_date && "bg-primary/5 z-10",
                date == @selected_date && "before:absolute before:inset-0 before:border-2 before:border-primary before:rounded-lg"
              ]}
            >
              ...
              <div class="flex justify-between items-start">
                <span class={[
                  ...
                  date == Events.today_br() && "bg-primary text-primary-content shadow-md shadow-primary/20",
                  date == @selected_date && date != Events.today_br() && "bg-base-300 text-base-content"
                ]}>
                  {date.day}
                </span>

                <%!-- Indicador de eventos para telas pequenas --%>
                <% day_events = Map.get(@events_by_date, date, []) %>
                <div :if={!Enum.empty?(day_events)} class="flex gap-0.5 sm:hidden">
                  <span class="size-1.5 rounded-full bg-primary animate-pulse"></span>
                </div>
              </div>

              <%!-- Lista de eventos para telas grandes --%>
              <div class="hidden sm:block mt-1.5 space-y-1 overflow-hidden">
                <div
                  :for={event <- Enum.take(day_events, 2)}
                  class={[
                    "text-[9px] px-2 py-0.5 rounded-md font-bold truncate transition-colors",
                    event.priority == "urgent" && "bg-red-500/10 text-red-600 border border-red-500/15",
                    event.priority == "important" && "bg-amber-500/10 text-amber-600 border border-amber-500/15",
                    event.priority == "normal" && "bg-primary/10 text-primary border border-primary/15"
                  ]}
                  title={"#{format_time(event.start_at)} - #{event.title}"}
                >
                  {format_time(event.start_at)} {event.church.name}
                </div>
                <div :if={length(day_events) > 2} class="text-[8px] text-center opacity-60 font-black tracking-wider uppercase">
        ...
        <div class="bg-base-100 border border-base-200/60 shadow-sm p-6 rounded-3xl relative overflow-hidden">
          <div class="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-primary/45 to-secondary/30" />
          ...
          <%= if Enum.empty?(selected_day_events) do %>
            <div class="text-center py-8 bg-base-200/20 rounded-2xl border border-dashed border-base-300">
              <p class="font-bold text-base-content opacity-60">
                Nenhum compromisso cadastrado para esta data.
              </p>
              <p class="text-xs opacity-50 mt-1">
                Utilize o calendário acima para verificar outros dias do mês.
              </p>
            </div>
          <% else %>
            <div class="space-y-4">
              ...
              >
                <div class="p-3 bg-gradient-to-br from-primary/10 to-secondary/5 text-primary rounded-xl shrink-0 border border-primary/10">
                ...
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <span class={"px-2.5 py-0.5 text-[9px] uppercase tracking-wider font-extrabold rounded-md #{p_class}"}>
                      {p_label}
                    </span>
                    <span class="px-2.5 py-0.5 text-[9px] uppercase tracking-wider font-extrabold rounded-md bg-base-300 text-base-content/85">
                      {translate_type(event.type)}
                    </span>
                  </div>
                  <p class="text-sm font-extrabold text-secondary mt-1.5 flex items-center gap-1">
                  ...
                  <%= if event.description && event.description != "" do %>
                    <div class="text-xs sm:text-sm text-base-content/80 mt-3 bg-base-100 border border-base-200/75 p-3.5 rounded-xl leading-relaxed">
                      {event.description}
                    </div>
                  <% end %>
                </div>
